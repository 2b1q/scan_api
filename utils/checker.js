...
const check_module_singleton = (() => {
  ...
  let initSingleton = () => {
    ...
    let check_Hash = (chash, hash, res) =>
      chash.length === 64 ? true : send_response(res, msg.bad_hash(hash), 404);

    // check addr from client request
    let check_addr = (caddr, addr, res) =>
      caddr.length === 40 ? true : send_response(res, msg.bad_addr(addr), 404);
    ...
    let check_block = (block, res) => {
      ...
      return block !== 0 ? true : send_response(res, msg.wrong_block, 404);
    };
    ...
    let cut_0x = hash =>
      typeof hash === "string" ? hash.split("0x").pop() : "";
    ...
    return {
      ...
      build_options: (req, lid, mid, eid) => build_params(req, lid, mid, eid), // build qury options
