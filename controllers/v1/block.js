...
// worker id pattern
const wid_ptrn = (() =>
    `${c.green}worker[${cluster.worker.id}]${c.cyan}[block controller][API v.1] ${c.white}`)();
...
const GetBlockTnx = async ({ listId, moduleId, page, size, entityId } = opts, res) => {
    ...
    try {
        ...
        if (res) res.status(400).json({ error: e });
        // FWD exception to client
        else return { error: e };
    }
};
...
const checkOptions = (req, listId = '', moduleId = 'block') => {
    ...
    if (listId.length > 0) {
        // check options for get block tnx Else check block param
        ...
        return check.block(block) ? check.build_options(req, listId, moduleId, block) : false;
    } else {
        let block = req.body.block || 0;
        block = Number(parseInt(block)); // convert to Number
        return check.block(block) ? block : false;
    }
};
