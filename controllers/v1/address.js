...
const ChekAddr = (clearAddr, entityId, res) =>
  new Promise(
    resolve =>
      check.checkAddr(clearAddr, entityId, res) ? resolve(true) : resolve(false)
  );
...
*/
const GetAddrTnx = async (
  { listId, moduleId, page, size, entityId } = opts,
  res
) => {
  ...
  options.collection = (listId =>
    listId === "listOfETH" ? "ether_txn" : "token_txn")(listId);
  logger.info({ addr: entityId, cleared_addr: clearAddr });
  // if we have res object -> its REST API else ITS socket IO data
  if (res)
    ChekAddr(clearAddr, entityId, res).then(async result => {
      if (result)
        res.json(await get_addr_tnxs(options, moduleId, listId, clearAddr));
...
const GetAddrTokensBalance = async (
  { listId, moduleId, skip, size, entityId } = opts,
  res
) => {
  ...
  if (res)
    ChekAddr(clearAddr, entityId, res).then(async result => {
      if (result)
        res.json(await get_addr_token_balance(options, moduleId, listId));
      else console.log(`result status is ${result}`);
    });
  // ITS socket IO data
