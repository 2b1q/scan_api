...
// worker id pattern
const wid_ptrn = (() =>
    `${c.green}worker[${cluster.worker.id}]${c.cyan}[list controller][API v.1] ${c.white}`)();
...
exports.list = (req, res) => {
    ...
    if (check.listId(listId) && check.moduleId(moduleId)) {
        ...
        res.status(400);
        res.json({
            error: `${check.get_msg().unknown_module_id.error} or ${
                check.get_msg().unknown_listid.error
            }`,
        });
    }
};
