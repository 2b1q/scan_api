...
const addrTokenBalance = async options => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (tokenCacheCol_p.rows === 0) await wait(50);
        ...
        tokenCacheCol_p.forEach(c_block => {
          ...
          if (c_block.lastblock > lastCachedBlock)
            lastCachedBlock = c_block.lastblock;
          return 0;
        });
        break;
      }
  ...
  let ctl_p = await dbquery.find(config.store.cols.erc20_cache, {
    addr: addr,
    lastblock: 0
  });
  ...
  lastTokens.forEach(t => {
    lastTokensPromiseList.push(
      dbquery.findOne(cfg.store.cols.token_head, { addr: t })
    );
  });
