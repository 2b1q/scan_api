...
const GetDbTransactions = async options => {
  ...
  return new Promise((resolve, reject) =>
    db_col
      .find(selector, { allowDiskUse: true }) // allowDiskUse lets the server know if it can use disk to store temporary results for the aggregation (requires mongodb 2.6 >)
      .sort(sort)
      .skip(skip)
      .limit(size)
      .toArray((err, docs) => {
...
const countTnx = async (collection, selector = {}) => {
  if (Array.isArray(collection)) {
    ...
    return await Promise.all(promise_all_result).then(data =>
      Object.assign(...data)
    );
  } else {
