...
// worker id pattern
const wid_ptrn = (() =>
  `${c.green}worker[${cluster.worker.id}]${c.cyan}[proxy] ${c.white}`)();
  ...
  let initSingleton = () => {
    ...
    return {
      ...
      getProvidersBlock: () => {
        ...
        let clients = self.ethClients;
        console.log(
          `${wid_ptrn}getProvidersBlock, clients length = ${id_ptrn(
            clients.length
          )}`
        );
        for (let i = 0; i < clients.length; i += 1) {
      ...
      getLastBlock: () => {
        ...
        console.log(
          `${wid_ptrn}getLastBlock, self.lastBlock = ${id_ptrn(self.lastBlock)}`
        );
